...
class AdminLockController extends Controller
{
    ...
    public function settings()
    {
        return $this->success([
            ...
            'password_a_set' => $this->lock->hasSimplePassword(),
            'password_b_set' => $this->lock->hasFullPassword(),
        ]);
    }
    public function updateSettings(Request $request)
    {
        $data = $request->validate([
            ...
            'password_a' => 'nullable|string|min:8|max:128', 'password_b' => 'nullable|string|min:12|max:128',
        ]);
        if (!$this->lock->verifyFull((string) $data['current_password'])) throw ValidationException::withMessages(['current_password' => '当前访问密码 B 不正确']);
        $simple = (string) ($data['password_a'] ?? ''); $full = (string) ($data['password_b'] ?? '');
        if ($simple !== '' && $full !== '' && hash_equals($simple, $full)) throw ValidationException::withMessages(['full_password' => '两个访问密码不能相同']);
        if ($simple !== '' && $full === '' && $this->lock->verifyFull($simple)) throw ValidationException::withMessages(['password_a' => '访问密码 A 不能与访问密码 B 相同']);
        if ($full !== '' && $simple === '' && $this->lock->verifySimple($full)) throw ValidationException::withMessages(['password_b' => '访问密码 B 不能与访问密码 A 相同']);
