...
const editorFields = computed(() => route.path === '/system/config'
  ? [{ key: 'key', label: '配置分组', readonly: true }, { key: 'value', label: '配置内容' }]
  : (config.value.editableKeys || Object.keys(form.value)));
...
function edit(row = {}) {
  const keys = config.value.editableKeys || Object.keys(row).filter((key) => !['created_at', 'updated_at', 'deleted_at'].includes(key));
  form.value = Object.fromEntries(keys.map((field) => typeof field === 'string' ? [field, row[field] ?? ''] : [field.key, row[field.key] ?? '']));
  draft.value = JSON.stringify(form.value, null, 2); advanced.value = false; showEditor.value = true;
}
async function save() {
  try {
    saving.value = true; error.value = '';
    let body = advanced.value ? JSON.parse(draft.value) : form.value;
    Object.keys(body).forEach((key) => body[key] === '' && key === 'password' && delete body[key]);
...
<template>
  <section class="page-stack">
    <div class="page-heading page-heading-row"><div><h1>{{ config.title }}</h1><p>数据直接来自 Xboard Plus 管理接口。</p></div><button v-if="config.save && config.create !== false" class="btn btn-primary" @click="edit()">新建</button></div>
    ...
    <section class="panel table-panel"><DataTable :rows="filteredRows" :columns="columns" :loading="loading" :editable="Boolean(config.save)" :deletable="Boolean(config.drop)" @edit="edit" @drop="drop" /></section>
    ...
    <div v-if="showEditor" class="modal-backdrop" @click.self="showEditor = false"><section class="modal-card"><div class="panel-head"><div><h2>编辑 {{ config.title }}</h2><p>仅提交当前模块允许修改的字段。</p></div><button class="btn btn-ghost" @click="showEditor = false">关闭</button></div><SmartEditor v-if="!advanced" v-model="form" :fields="editorFields" /><textarea v-else v-model="draft" class="json-editor" spellcheck="false" /><div class="modal-actions split-actions"><button v-if="route.path !== '/system/config'" class="btn btn-ghost" @click="advanced = !advanced; draft = JSON.stringify(form, null, 2)">{{ advanced ? '返回表单' : '高级 JSON' }}</button><span v-else></span><button class="btn btn-primary" :disabled="saving" @click="save">{{ saving ? '保存中…' : '保存' }}</button></div></section></div>
  </section>
</template>
