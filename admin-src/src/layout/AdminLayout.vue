...
async function loadLock() { try { const data = await get('/admin-lock/status'); lockEnabled.value = Boolean(data.enabled); lockScope.value = data.scope || 'locked'; } catch { lockEnabled.value = true; lockScope.value = 'locked'; } finally { lockReady.value = true; resetIdleTimer(); } }
...
<template>
  <div v-if="lockReady" class="admin-shell" :class="{ 'sidebar-collapsed': collapsed, 'mobile-nav-open': mobileOpen, 'gate-active': lockEnabled && lockScope !== 'b' }">
    <AdminSidebar v-if="!lockEnabled || lockScope === 'b'" :collapsed="collapsed" @close="mobileOpen = false" />
