...
function extensions(field) {
  ...
  return [basicSetup, lintGutter(), keymap.of([indentWithTab]), ...language,
    ...
    EditorView.theme({
      '&': { height: '620px', backgroundColor: 'transparent', fontSize: '13px' },
      ...
      '.cm-activeLine, .cm-activeLineGutter': { backgroundColor: 'rgba(103, 80, 255, .06)' },
...
onMounted(() => { activeKey.value ||= entries.value[0]?.key || ''; nextTick(createEditor); });
...
<template>
  <div class="template-code-editor">
    <div class="template-tabs" role="tablist" aria-label="订阅模板类型">
      <button v-for="field in entries" :key="field.key" type="button" :class="{ active: field.key === activeKey }" role="tab" :aria-selected="field.key === activeKey" @click="select(field.key)">{{ field.tabLabel || field.label.replace(' 模板', '') }}</button>
    </div>
    <div class="template-editor-head">
      ...
      <div class="template-editor-tools"><span class="language-badge">{{ active?.language?.toUpperCase() }}</span><button v-if="active?.language === 'json'" type="button" class="btn btn-ghost btn-sm" @click="format">格式化</button></div>
    </div>
